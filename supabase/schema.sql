...
create table if not exists public.accounts (
  ...
  profile_image text,
  monthly_goal_pct numeric not null default 0,
  yearly_goal_pct numeric not null default 0,
  daily_loss_limit_pct numeric not null default 0,
  monthly_loss_limit_pct numeric not null default 0,
  position_size_enabled boolean not null default false,
  base_currency text not null default 'USD',
  default_risk_pct numeric not null default 1,
  default_stop_loss_pips numeric not null default 0,
  created_at     timestamptz not null default now(),
  updated_at     timestamptz not null default now()
);
...
alter table public.user_settings add column if not exists custom_instruments jsonb not null default '[]'::jsonb;
...
update public.trades set entry_session = session where entry_session is null and session is not null;
alter table public.trades alter column rating type numeric using rating::numeric;
