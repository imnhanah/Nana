...
# September journal update

## Challenge modes

Run `supabase/challenge_modes_migration.sql` once after the existing Challenge table is installed. This adds one nullable JSON column; it does not alter trades, balances, levels, or ownership policies.

In Account Settings → Challenge → Challenge Mode, select Mode 1 (Risk Limit) or Mode 2 (Two-Loss Streak). Click the selected mode again to return to manual. The account must already have Challenge enabled and saved. Mode and current-level changes save immediately, independently of the other account settings. The same controls are available on the Challenge page.

Both modes accumulate signed Gross P&L at the current level. Mode 1 steps back at the cumulative risk limit. Mode 2 steps back after two negative Gross P&L trades: positive Gross P&L resets the streak and zero leaves it unchanged. Transitions discard excess and reset the level total/streak. A trade can cause only one step. Level 1 cannot step lower; passing Level 30 completes the challenge.

Modes/manual level changes create a saved reset point. Previously closed trades are excluded, even if later edited. Trades must have a close date after that reset point to count; same-day date-only closes entered afterward are eligible and sort after timed closes. Equal closing timestamps sort by trade ID. Edits, deletions, or removal of a close date recalculate all eligible trades since the latest reset point, not frozen progress before it.

The database stores the manual baseline and reset point; current automated progress is deterministically recalculated from saved trades whenever the account loads or trades change. No background job or repeated advancement write is needed. Disabling Challenge switches it back to manual and preserves its current level and notes. Notes save when leaving their field.

Before using close dates/times, run `supabase/trade_close_timing_migration.sql` in your Supabase SQL Editor. It adds optional columns without deleting existing trades. It is safe to run again. The existing trade date/time remain the opening date/time; calendar placement continues to use the opening date.

Page headings are calculated locally from the active journal's saved records. They report patterns and missing information, not predictions or proof of an edge. Delete controls turn into an inline red Confirm button; a second click is required. Clicking elsewhere, leaving the button, Escape, or six seconds without confirmation cancels it.

Trade and markup pickers float over the form without changing its spacing. Open/close date and time share one row on desktop, with wider date fields; narrow screens use two date/time pairs. Only the Dashboard keeps the AAICOREFX / DASHBOARD label, linking to the landing page. This interface update needs no additional SQL.
