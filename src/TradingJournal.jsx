...
import { SharedCalendar, AccountGuardrails } from "./ReferenceUI";
import AnalyticsDeck from "./AnalyticsDeck";
import { REFERENCE_STYLES } from "./referenceStyles";
import ProfileSystemModal from "./ProfileSystemModal";
  ...
  return (
    <div className="tj-field">
      ...
      <div
        ...
        tabIndex={0}
        aria-label="Upload or paste screenshots into this section"
        onPaste={(event) => { const files = Array.from(event.clipboardData?.files || []).filter(file => file.type.startsWith("image/")); if (files.length) { event.preventDefault(); event.stopPropagation(); addFiles(files); } }}
...
function ListManager({ title, items, onSave, note }) {
  ...
  const rename = (old) => { const next=(editing?.value || "").trim(); if(next && !items.some(x=>x !== old && x.toLowerCase()===next.toLowerCase())) onSave(items.map(x=>x===old?next:x)); setEditing(null); };
...
function RuleManager({ rules, onAdd, onUpdate, onRemove }) {
  const [value,setValue]=useState(""),[editing,setEditing]=useState(null);
  const add=()=>{if(value.trim()){onAdd(value.trim());setValue("");}};
  const save=()=>{if(editing?.text.trim()){onUpdate(editing.id,editing.text.trim());setEditing(null);}};
  return <Card className="tj-panel"><h3>Rule Management</h3><p className="tj-muted-txt">Controls the rule checklist shown in Log Trade and your daily discipline workflow.</p><div className="tj-inline-add"><input className="tj-input" placeholder="Add trading rule..." value={value} onChange={e=>setValue(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()}/><button className="tj-btn-primary" onClick={add}>Add</button></div><div className="tj-rule-list">{rules.map(rule=><div key={rule.id} className="tj-rule-row">{editing?.id===rule.id?<input autoFocus className="tj-input" value={editing.text} onChange={e=>setEditing({...editing,text:e.target.value})} onKeyDown={e=>{if(e.key==="Enter")save();if(e.key==="Escape")setEditing(null);}}/>:<span>{rule.text}</span>}<span className="tj-rule-actions"><button className="tj-icon-btn" title={editing?.id===rule.id?"Save rule":"Edit rule"} onClick={()=>editing?.id===rule.id?save():setEditing({id:rule.id,text:rule.text})}>{editing?.id===rule.id?<CheckCircle2 size={14}/>:<Pencil size={14}/>}</button>{editing?.id===rule.id&&<button className="tj-icon-btn" title="Cancel" onClick={()=>setEditing(null)}><X size={14}/></button>}<button className="tj-icon-btn" title="Delete rule" onClick={()=>onRemove(rule.id)}><Trash2 size={14}/></button></span></div>)}</div></Card>;
}
function ManagementPage({ typeTags, mistakeTags, confluenceSessions, instruments, onTypeTags, onMistakes, onConfluence, onInstruments, rules, onAddRule, onUpdateRule, onRemoveRule }) {
  return <div className="tj-management-workspace"><div className="tj-page-intro"><div><h3>Management</h3><p className="tj-muted-txt">Configure the options available on future trades and markups. Historical records remain unchanged.</p></div></div><div className="tj-management-grid">
    <InstrumentManager instruments={instruments} onSave={onInstruments}/>
    <ListManager title="Tag Management" items={typeTags} onSave={onTypeTags} note="Setup confluences available in Log Trade."/>
    <ListManager title="Entry Model Management" items={confluenceSessions} onSave={onConfluence} note="Single-select options shown in Log Trade."/>
    <ListManager title="Mistake Management" items={mistakeTags} onSave={onMistakes} note="Multi-select mistakes available in Log Trade."/>
    <RuleManager rules={rules} onAdd={onAddRule} onUpdate={onUpdateRule} onRemove={onRemoveRule}/>
  </div></div>;
}
...
function MarkupModal({ onClose, onSave, editing, instruments = [] }) {
  ...
  return <Modal title={editing ? "Edit Premarket Markup" : "New Premarket Markup"} onClose={onClose} wide>
    ...
    <div className="tj-markup-section"><div className="tj-section-label">Post-session D1</div><ScreenshotUploader max={2} screenshots={f.screenshots.postD1} onChange={setShots("postD1")}/></div>
...
function ReferenceMarkupsPage({ markups, trades, onNew, onEdit, onDelete, onTrade }) {
  ...
  const slots = [["preM15", "Pre-session M15"], ["preH4", "Pre-session H4"], ["postD1", "Post-session D1"]];
  ...
  const postCoverage = coverage(countWith(["postD1"]));
...
function PeriodReviewModal({ period, saved, reviews, onClose, onSave, onStartTradeReview }) {
  ...
  return <Modal title={`${period.label} Review`} onClose={onClose} wide>
    ...
    <section className="tj-period-trades"><div className="tj-period-trades-head"><div><div className="tj-bold">Trades Taken</div><div className="tj-muted-txt" style={{ fontSize: 12 }}>Review coverage stays linked to the original trade record.</div></div><span className="tj-count-badge">{stat.total}</span></div>{stat.trades.length ? [...stat.trades].sort((a, b) => `${b.date} ${b.time || ""}`.localeCompare(`${a.date} ${a.time || ""}`)).map((trade) => { const review = reviewByTrade.get(trade.id); const result = Number(trade.pnl) > 0 ? "WIN" : Number(trade.pnl) < 0 ? "LOSS" : "B/E"; return <div className="tj-period-trade-row" key={trade.id}><div><strong>{trade.asset || "No instrument"}</strong><span className={`tj-dirpill-sm ${trade.direction === "BUY" ? "tj-green" : "tj-red"}`}>{trade.direction}</span><span className="tj-sesspill">{trade.entrySession || trade.session || "—"}</span><span className="tj-sesspill">{trade.entryType || trade.confluenceSession || "—"}</span><small>{trade.date} · {trade.rr ? `${Number(trade.rr).toFixed(2)}R` : "—"}</small></div><div><span className={`tj-statuspill tj-statuspill-${result === "WIN" ? "win" : result === "LOSS" ? "loss" : "be"}`}>{result}</span><span className={review ? "tj-review-status-done" : "tj-review-status-pending"}>{review ? "✓ Reviewed" : "Review Pending"}</span><button className="tj-btn-outline tj-btn-small" onClick={() => onStartTradeReview(trade, review)}>{review ? "Edit" : "Review"}</button></div></div>; }) : <div className="tj-empty">No trades were logged for this period.</div>}</section>
    <div className="tj-section-label tj-reference-review-label">Review</div>
...
function ReferenceDashboardPage({ account, stats, monthCursor, setMonthCursor, onDayClick, guardrails }) {
  ...
  const recentTrades = [...account.trades].sort((a, b) => `${b.date} ${b.time || ""}`.localeCompare(`${a.date} ${a.time || ""}`));
  ...
  const lastSixBase = Number(account.balance) + stats.netPnl - lastSixPnl;
  const flowPct = lastSixBase > 0 ? lastSixPnl / lastSixBase * 100 : 0;
  ...
  return <div className="tj-reference-dashboard">
    <AccountGuardrails account={account} guardrails={guardrails}/>
    ...
    <div className="tj-reference-hero-grid">
      <SharedCalendar account={account} monthCursor={monthCursor} setMonthCursor={setMonthCursor} onDayClick={onDayClick}/>
      <Card className="tj-panel tj-reference-flow">
        <div className="tj-reference-flow-head"><div><strong>{view === "recent" ? "Recent Trades" : "Performance Flow"}</strong><span>Current month pulse across closes, strongest context, and recent outcomes.</span></div><div className="tj-tabs"><button className={`tj-tab ${view === "recent" ? "tj-tab-active" : ""}`} onClick={() => setView("recent")}>Recent Trades</button><button className={`tj-tab ${view === "flow" ? "tj-tab-active" : ""}`} onClick={() => setView("flow")}>Performance Flow</button></div></div>
        {view === "recent" ? <div className="tj-recent-table"><div className="tj-recent-head"><span>Close Date</span><span>Symbol</span><span>Side</span><span>Session</span><span>Net P&amp;L</span></div>{recentTrades.length ? recentTrades.map((trade) => <div className="tj-recent-row" key={trade.id}><span>{trade.date} · {formatTime(trade.time)}</span><strong>{trade.asset}</strong><span className={trade.direction === "BUY" ? "tj-green" : "tj-red"}>{trade.direction}</span><span>{trade.entrySession || trade.session || "—"}</span><strong className={trade.pnl >= 0 ? "tj-green" : "tj-red"}>{fmtMoney(trade.pnl)}</strong></div>) : <div className="tj-empty">No trades yet. Your newest completed trade will appear here.</div>}</div> : <><div className="tj-reference-flow-metrics"><div><span>LAST 6 CLOSES</span><strong className={flowPct >= 0 ? "tj-green" : "tj-red"}>{flowPct >= 0 ? "+" : ""}{flowPct.toFixed(2)}%</strong><small>{fmtMoney(lastSixPnl)}</small></div><div><span>BEST SESSION</span><strong>{bestSession?.session || "—"}</strong><small className={bestSession?.pnl >= 0 ? "tj-green" : "tj-red"}>{bestSession ? fmtMoney(bestSession.pnl) : "No trades"}</small></div><div><span>HOT PAIR</span><strong>{hotPair?.[0] || "—"}</strong><small className={hotPair?.[1] >= 0 ? "tj-green" : "tj-red"}>{hotPair ? fmtMoney(hotPair[1]) : "No trades"}</small></div><div><span>CURRENT STREAK</span><strong className={stats.streakType === "loss" ? "tj-red" : "tj-green"}>{stats.streak ? `${stats.streakType === "loss" ? "-" : "+"}${stats.streak}` : "—"}</strong><small>{lastSixPositive}/{lastSix.length || 0} recent closes green</small></div></div><div className="tj-reference-flow-chart"><ResponsiveContainer width="100%" height={170}><AreaChart data={monthFlow}><defs><linearGradient id="dashboardFlow" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={UI_COLORS.primary} stopOpacity={.26}/><stop offset="100%" stopColor={UI_COLORS.primary} stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="var(--tj-chart-grid)" strokeDasharray="3 3" vertical={false}/><XAxis dataKey="date" tick={CHART_TICK} axisLine={false} tickLine={false}/><YAxis hide/><Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(value) => [fmtMoney(value), "Flow"]}/><Area type="monotone" dataKey="pnl" stroke={UI_COLORS.primary} fill="url(#dashboardFlow)" strokeWidth={2.25} dot={false}/></AreaChart></ResponsiveContainer></div><div className="tj-reference-flow-trades">{recentTrades.slice(0, 3).map((trade) => <div key={trade.id}><span><strong>{trade.asset}</strong><small>{trade.direction} · {trade.entrySession || trade.session || "—"} · {trade.date.slice(5)} · {formatTime(trade.time)}</small></span><span><small>{trade.rr ? Number(trade.rr).toFixed(2) + "R" : "—"}</small><b className={trade.pnl >= 0 ? "tj-green" : "tj-red"}>{fmtMoney(trade.pnl)}</b></span></div>)}</div></>}
      </Card>
    </div>
...
function TradeLogPage({ account, reviews = [], markups = [], onEdit, onDelete, onNewTrade }) {
  ...
  const evidenceCoverage = stats.total ? account.trades.filter((trade) => trade.screenshots?.length).length / stats.total * 100 : 0;
  const markupCoverage = stats.total ? account.trades.filter((trade) => trade.premarketMarkupId).length / stats.total * 100 : 0;
  const timeCoverage = stats.total ? account.trades.filter((trade) => trade.time).length / stats.total * 100 : 0;
  ...
  return (
    <>
      <Card className="tj-panel ref-capture-quality">
        <div className="ref-section-head"><div><div className="ref-eyebrow">Capture Quality</div><h3>Journal evidence coverage</h3><p>See whether each trade carries the context needed for a useful review.</p></div><span className="ref-badge">{stats.total} trades tracked</span></div>
        <div className="ref-capture-grid">{[["Evidence", evidenceCoverage, "Trade screenshots"], ["Markup Links", markupCoverage, "Premarket plans attached"], ["Time Capture", timeCoverage, "Stored execution times"]].map(([label, value, note]) => <div key={label}><span>{label}</span><strong>{value.toFixed(0)}%</strong><div className="tj-bar-track"><i className="tj-bar-fill tj-bar-green" style={{ width: `${value}%` }}/></div><small>{note}</small></div>)}</div>
      </Card>
              ...
              <Card key={t.id} className={`tj-tlog-card tj-reference-tradelog-row tj-tlog-${cls}`}>
                ...
                {isOpen && (
                  <div className="tj-tlog-expand">
                    ...
                    <section className="tj-reference-trade-brief"><div className="tj-section-label">Trade Brief</div><div className="tj-reference-brief-grid"><div><span>RESULT</span><strong className={t.pnl >= 0 ? "tj-green" : "tj-red"}>{cls === "win" ? "Win" : cls === "loss" ? "Loss" : "Breakeven"}</strong></div><div><span>ENTRY MODEL</span><strong>{t.entryType || t.confluenceSession || "—"}</strong></div><div><span>MOOD SHIFT</span><strong>{t.moodBefore || "—"} → {t.moodAfter || "—"}</strong></div><div><span>RATING</span><strong><RatingDisplay value={t.rating} noRules={!t.ruleEvaluations?.length&&!t.rating} /></strong></div></div>{t.context && <p>{t.context}</p>}</section>
                    ...
                    {linkedMarkup && [
                      ["Pre-session M15", linkedMarkup.screenshots?.preM15],
                      ["Pre-session H4", linkedMarkup.screenshots?.preH4],
                      ["Post-session D1", linkedMarkup.screenshots?.postD1],
                    ].some(([, images]) => images?.length) && <div className="tj-reference-markup-shots">{[
                      ["Pre-session M15", linkedMarkup.screenshots?.preM15],
                      ["Pre-session H4", linkedMarkup.screenshots?.preH4],
                      ["Post-session D1", linkedMarkup.screenshots?.postD1],
                    ].flatMap(([label, images]) => (images || []).map((src, index) => <div key={`${label}-${index}`}><span>{label}</span><ImagePreview src={src} alt={`${label} markup chart`} /></div>))}</div>}
...
function CalendarPage({ account, monthCursor, setMonthCursor, onDayClick }) {
  ...
  return <div className="ref-page-stack">
    <AccountGuardrails account={account} guardrails={accountGuardrails(account, account.trades)}/>
    <div className="tj-cal-tabs">{["calendar","monthly","weekly","daily"].map(tab=><button key={tab} className={`tj-newstab ${calTab===tab?"tj-newstab-active":""}`} onClick={()=>setCalTab(tab)}>{tab[0].toUpperCase()+tab.slice(1)}</button>)}</div>
    {calTab==="calendar"&&<SharedCalendar account={account} monthCursor={monthCursor} setMonthCursor={setMonthCursor} onDayClick={onDayClick} filters/>}
    {calTab==="monthly"&&<PerformanceTable title="Monthly Performance" unitLabel="month" rows={computeMonthlyPerformance(account.trades,account.breakevenCap,account.balance)}/>}
    {calTab==="weekly"&&<PerformanceTable title="Weekly Performance" unitLabel="week" rows={computeWeeklyPerformanceAll(account.trades,account.breakevenCap,account.balance)}/>}
    {calTab==="daily"&&<DailyPerformanceView trades={account.trades} cap={account.breakevenCap}/>}
  </div>;
}
...
function RulesPage({ account, onToggleCheckin, onAddRule, onUpdateRule, onRemoveRule, onManage }) {
    ...
    <Card className="tj-panel">
      <div className="tj-rules-head">
        ...
        <div className="tj-tabs">
          ...
          <button className={`tj-tab ${tab === "manage" ? "tj-tab-active" : ""}`} onClick={onManage}>⚙ Manage</button>
        </div>
      </div>
      {tab === "today" && (activeRules.length === 0 ? (
        <div className="tj-empty-block">
          ...
          <button className="tj-btn-primary" onClick={onManage}><Plus size={14} /> Add Rules</button>
        </div>
      ) : (
...
function TradingJournalApp({ user, onLogout }) {
  ...
  useEffect(() => {
    const minutes = Number(user.user_metadata?.session_timeout) || 0;
    if (!minutes) return undefined;
    let timer;
    const arm = () => { window.clearTimeout(timer); timer = window.setTimeout(() => onLogout(), minutes * 60 * 1000); };
  ...
  }, [user.user_metadata?.session_timeout, onLogout]);
  ...
  if (!loaded || !account || !stats) {
    return (
      <div className="tj-root tj-loading">
        <style>{CSS + REFERENCE_STYLES}</style>
    ...
    <div className={`tj-root ${account.theme === "light" ? "tj-theme-light" : "tj-theme-dark"}`}>
      ...
      <div className={`tj-sidebar ${sidebarOpen ? "tj-sidebar-shown" : "tj-sidebar-collapsed"}`}>
        <div className="tj-sidebar-scroll">
          <button className="ref-sidebar-profile" onClick={() => setModal("profile")} aria-label="Open Profile and System settings"><span className="tj-avatar">{user.user_metadata?.profile_image || account.profileImage ? <img src={user.user_metadata?.profile_image || account.profileImage} alt="" /> : <img src={logoUrl} alt="AAICOREFX" />}</span><span><b>{displayName}</b><small>Profile &amp; System</small></span></button>
          ...
          <div className="tj-nav">
            <button className={`tj-nav-item ${modal === "profile" ? "tj-nav-active" : ""}`} onClick={() => setModal("profile")}><Settings size={16} /> <span>Profile &amp; System</span></button>
            <button className={`tj-nav-item ${modal === "editaccount" ? "tj-nav-active" : ""}`} onClick={() => setModal("editaccount")}><SlidersHorizontal size={16} /> <span>Account Settings</span></button>
          ...
          <div className="tj-logged-in-as">Signed in as <span className="tj-bold">{displayName}</span></div>
        </div>
      ...
      <div className="tj-main">
        ...
        <div className="tj-content">
          ...
          {page === "analytics" && <AnalyticsDeck account={account} stats={stats} computeStats={computeStats} guardrails={guardrails}/>}
          ...
          {page === "news" && <NewsPage />}
          {page === "rules" && <RulesPage account={account} onManage={()=>setPage("management")} onToggleCheckin={handleToggleCheckin} onAddRule={handleAddRule} onUpdateRule={handleUpdateRule} onRemoveRule={handleRemoveRule} />}
          {page === "management" && <ManagementPage typeTags={typeTags} mistakeTags={mistakeTags} confluenceSessions={confluenceSessions} instruments={customInstruments} onTypeTags={(x)=>saveList("types",x)} onMistakes={(x)=>saveList("mistakes",x)} onConfluence={(x)=>saveList("confluence",x)} onInstruments={(x)=>saveList("instruments",x)} rules={account.rules} onAddRule={handleAddRule} onUpdateRule={handleUpdateRule} onRemoveRule={handleRemoveRule} />}
      ...
      {modal === "profile" && <ProfileSystemModal user={user} account={account} onClose={() => setModal(null)} onSaveAccount={handleSaveAccountSettings} onLogout={onLogout} onMessage={showInfo} />}
