...
import {
  ...
  createTrade, updateTrade, deleteTrade, createRule, deleteRule, setCheckin,
  saveTypeTags, saveManagedLists, createMarkup, deleteMarkup, saveTradeReview, hasMigratedLocalData, markLocalDataMigrated, importLegacyAccount,
} from "./db";
...
const wrHex = (wr) => (wr > 50 ? "#4DFF66" : wr >= 30 ? "#FBBF24" : "#F87171");
...
const GRADE_COLORS = ["#F87171", "#FBBF24", "#60A5FA", "#4DFF66", "#4DFF66"];
...
function RatingDisplay({ ratingStyle, value }) {
  return ratingStyle === "grades" ? <GradeRating value={value} size="sm" /> : <span>{"★".repeat(value)}{"☆".repeat(5 - value)}</span>;
}
...
function NewTradeModal({ onClose, onSave, editing, typeTags, mistakeTags, confluenceSessions, markups, rules, defaultCommission, ratingStyle }) {
  const [form, setForm] = useState(
    editing || {
      id: uid(), date: todayISO(), asset: "", direction: "BUY", grossPnl: "", commission: defaultCommission || 0, swap: 0, pnl: "", rr: "",
      session: SESSIONS[2], rating: 0, types: [], mistakes: [], moodBefore: "Neutral",
      moodAfter: "Neutral", context: "", screenshots: [], confluenceSession: "", premarketMarkupId: null, ruleEvaluations: [],
    }
  );
  ...
  const save = () => {
    if (!form.asset.trim() || form.grossPnl === "") return;
    const grossPnl = Number(form.grossPnl); const commission = Number(form.commission) || 0; const swap = Number(form.swap) || 0;
    if (!Number.isFinite(grossPnl)) return;
    onSave({ ...form, grossPnl, commission, swap, pnl: grossPnl - commission - swap, rr: parseFloat(form.rr) || 0 });
  };
    ...
    <Modal title={editing ? "Edit Trade" : "New Trade"} onClose={onClose} wide>
      ...
      <div className="tj-grid3">
        <Field label="Gross P&L ($) *">
          <input type="number" className="tj-input" placeholder="-50" value={form.grossPnl}
            onChange={(e) => set("grossPnl", e.target.value)} />
        </Field>
        <Field label="Commission ($)"><input type="number" className="tj-input" value={form.commission} onChange={(e) => set("commission", e.target.value)} /></Field>
        <Field label="Swap ($)"><input type="number" className="tj-input" value={form.swap} onChange={(e) => set("swap", e.target.value)} /></Field>
      </div>
      <div className="tj-grid3">
        ...
        <Field label="RR"><input type="number" step="0.1" className="tj-input" placeholder="2.5" value={form.rr} onChange={(e) => set("rr", e.target.value)} /></Field>
        <Field label="Session">
      ...
      <div className="tj-section-label">Context & Confluence</div>
      <div className="tj-grid2">
        <Field label="Confluence Session"><select className="tj-input" value={form.confluenceSession} onChange={(e) => set("confluenceSession", e.target.value)}><option value="">None</option>{confluenceSessions.map((x) => <option key={x} value={x}>{x}</option>)}</select></Field>
        <Field label="Premarket Markup"><select className="tj-input" value={form.premarketMarkupId || ""} onChange={(e) => set("premarketMarkupId", e.target.value || null)}><option value="">None</option>{markups.map((m) => <option key={m.id} value={m.id}>{m.date} · {m.instrument || m.market || "Untitled"} · {m.bias}</option>)}</select></Field>
      </div>

      <div className="tj-section-label">Rating</div>
      {ratingStyle === "grades"
        ? <GradeRating value={form.rating} onChange={(v) => set("rating", v)} />
      ...
      {rules.length ? <div className="tj-rule-list">{rules.filter((r) => r.active).map((r) => { const current = form.ruleEvaluations.find((x) => x.ruleId === r.id); return <label key={r.id} className="tj-rule-row"><span><input type="checkbox" checked={!!current?.checked} onChange={(e) => setForm((f) => ({...f, ruleEvaluations: [...f.ruleEvaluations.filter((x) => x.ruleId !== r.id), { ruleId: r.id, name: r.text, checked: e.target.checked }]}))} /> {r.text}</span>{current?.checked && <CheckCircle2 size={16} color="var(--tj-green)" />}</label>; })}</div> : <div className="tj-empty">No active rules. Add them from Rules.</div>}
...
function ManagementPage({ typeTags, mistakeTags, confluenceSessions, onTypeTags, onMistakes, onConfluence }) { return <><div className="tj-page-intro"><div className="tj-bold" style={{fontSize:18}}>Management</div><div className="tj-muted-txt" style={{fontSize:12}}>Configure the options available on future trades. Historical records remain unchanged.</div></div><div className="tj-row3"><ListManager title="Tag Management" items={typeTags} onSave={onTypeTags} note="Setup tags available in New Trade." /><ListManager title="Confluence Sessions" items={confluenceSessions} onSave={onConfluence} note="Independent from tags and analytics." /><ListManager title="Mistake Management" items={mistakeTags} onSave={onMistakes} note="Multi-select mistakes available in New Trade." /></div></>; }

function MarkupModal({ onClose, onSave }) {
  const [f,setF]=useState({date:todayISO(),market:"",instrument:"",bias:"",levels:"",structure:"",notes:"",screenshots:{preM15:[],preH4:[],postD1:[],postH4:[],postM15:[]}});
  const set=(k,v)=>setF(x=>({...x,[k]:v}));
  const setShots=(slot)=>(updater)=>setF(x=>({...x,screenshots:{...x.screenshots,[slot]:updater(x.screenshots[slot])}}));
  return <Modal title="New Premarket Markup" onClose={onClose} wide>
    <div className="tj-grid3"><Field label="Date"><input type="date" className="tj-input" value={f.date} onChange={e=>set("date",e.target.value)}/></Field><Field label="Market / Session"><input className="tj-input" placeholder="NY AM" value={f.market} onChange={e=>set("market",e.target.value)}/></Field><Field label="Instrument"><input className="tj-input" placeholder="EUR/USD" value={f.instrument} onChange={e=>set("instrument",e.target.value)}/></Field></div>
    <div className="tj-markup-section"><div className="tj-section-label">Pre-Session Analysis</div><div className="tj-grid2"><Field label="Bias"><input className="tj-input" placeholder="Bullish / Bearish / Neutral" value={f.bias} onChange={e=>set("bias",e.target.value)}/></Field><Field label="Key levels, liquidity & zones"><input className="tj-input" value={f.levels} onChange={e=>set("levels",e.target.value)}/></Field></div><Field label="Market structure / narrative"><textarea className="tj-input tj-textarea" placeholder="HTF context, structure and key areas..." value={f.structure} onChange={e=>set("structure",e.target.value)}/></Field><div className="tj-grid2"><div><div className="tj-field-label">LTF (M15) chart</div><ScreenshotUploader screenshots={f.screenshots.preM15} onChange={setShots("preM15")}/></div><div><div className="tj-field-label">MTF (H4) chart</div><ScreenshotUploader screenshots={f.screenshots.preH4} onChange={setShots("preH4")}/></div></div></div>
    ...
    <div className="tj-modal-actions"><button className="tj-btn-outline" onClick={onClose}>Cancel</button><button className="tj-btn-primary" onClick={()=>onSave(f)}>Save Markup</button></div>
  </Modal>;
}
function MarkupsPage({ markups, trades, onNew, onDelete }) { const [open,setOpen]=useState({}); return <><div className="tj-rules-head"><div><div className="tj-bold" style={{fontSize:16}}>Premarket Markups</div><div className="tj-muted-txt" style={{fontSize:12}}>Prepare context before execution, then attach it to a trade.</div></div><button className="tj-btn-primary" onClick={onNew}><Plus size={15}/> New Markup</button></div><div className="tj-tlog-list">{markups.length?markups.map(m=>{const linked=trades.filter(t=>t.premarketMarkupId===m.id);const pnl=linked.reduce((s,t)=>s+t.pnl,0);return <Card key={m.id} className="tj-tlog-card"><div className="tj-tlog-row" onClick={()=>setOpen(x=>({...x,[m.id]:!x[m.id]}))}><div className="tj-tlog-main"><div className="tj-tlog-asset">{m.instrument||m.market||"Untitled markup"} <span className="tj-sesspill">{m.bias||"No bias"}</span></div><div className="tj-muted-txt" style={{fontSize:12}}>{m.date} · {m.market||"No session"} · {linked.length} linked trade{linked.length===1?"":"s"}</div></div><div className={`tj-tlog-pnl ${pnl>=0?"tj-green":"tj-red"}`}>{fmtMoney(pnl)}</div><button className="tj-icon-btn" onClick={(e)=>{e.stopPropagation();onDelete(m.id)}}><Trash2 size={14}/></button><ChevronDown size={16} style={{transform:open[m.id]?"rotate(180deg)":"none"}}/></div>{open[m.id]&&<div className="tj-tlog-detail"><div className="tj-tlog-detail-grid"><div><div className="tj-mlabel">LEVELS / ZONES</div><div>{m.levels||"—"}</div></div><div><div className="tj-mlabel">STRUCTURE</div><div>{m.structure||"—"}</div></div><div><div className="tj-mlabel">EXPECTATIONS</div><div>{m.notes||"—"}</div></div></div><div className="tj-section-label">Linked Trades</div>{linked.length?linked.map(t=><div key={t.id} className="tj-rule-row"><span>{t.date} · {t.asset} · {t.direction}</span><span className={t.pnl>=0?"tj-green":"tj-red"}>{fmtMoney(t.pnl)}</span></div>):<div className="tj-empty">No trades linked yet.</div>}</div>}</Card>}):<div className="tj-empty-block"><ScanLine size={32}/><div className="tj-empty-title">No premarket markups</div><button className="tj-btn-primary" onClick={onNew}>Create your first markup</button></div>}</div></>; }
function ReviewsPage({ reviews, trades, onSave }) {
  const empty = { tradeId:"", date:todayISO(), doneWell:"", wentWrong:"", execution:"", adherence:"", psychology:"", lessons:"", actions:"", notes:"", screenshots:[] };
  const [form, setForm] = useState(empty);
  const [open, setOpen] = useState({});
  const set = (key, value) => setForm((f) => ({ ...f, [key]: value }));
  const save = () => { if (!form.tradeId) return; onSave(form); setForm({ ...empty, date: todayISO() }); };
  return <>
    <Card className="tj-panel"><div className="tj-bold" style={{fontSize:16}}>Trade Review</div><div className="tj-muted-txt" style={{fontSize:12,margin:"4px 0 14px"}}>Capture execution, rule adherence and the next improvement—not just the result.</div>
      <div className="tj-grid2"><Field label="Trade reference *"><select className="tj-input" value={form.tradeId} onChange={(e)=>set("tradeId",e.target.value)}><option value="">Select a completed trade</option>{trades.map((t)=><option key={t.id} value={t.id}>{t.date} · {t.asset} · Net {fmtMoney(t.pnl)}</option>)}</select></Field><Field label="Review date"><input type="date" className="tj-input" value={form.date} onChange={(e)=>set("date",e.target.value)} /></Field></div>
    ...
    <div className="tj-tlog-list" style={{marginTop:14}}>{reviews.length?reviews.map((r)=>{const t=trades.find((x)=>x.id===r.tradeId);return <Card key={r.id} className="tj-tlog-card"><div className="tj-tlog-row" onClick={()=>setOpen(x=>({...x,[r.id]:!x[r.id]}))}><div className="tj-tlog-main"><div className="tj-tlog-asset">{t?.asset||"Historical trade"} {t&&<span className={`tj-dirpill-sm ${t.direction==="BUY"?"tj-green":"tj-red"}`}>{t.direction}</span>}</div><div className="tj-muted-txt" style={{fontSize:12}}>{r.date} · Net P&amp;L {t?fmtMoney(t.pnl):"—"}</div></div>{t&&<RatingDisplay ratingStyle="stars" value={t.rating}/>}<ChevronDown size={16} style={{transform:open[r.id]?"rotate(180deg)":"none"}}/></div>{open[r.id]&&<div className="tj-tlog-detail"><div className="tj-tlog-detail-grid"><div><div className="tj-mlabel">WHAT WENT WELL</div><div>{r.doneWell||"—"}</div></div><div><div className="tj-mlabel">WHAT WENT WRONG</div><div>{r.wentWrong||"—"}</div></div><div><div className="tj-mlabel">EXECUTION</div><div>{r.execution||"—"}</div></div><div><div className="tj-mlabel">RULE ADHERENCE</div><div>{r.adherence||"—"}</div></div></div><div className="tj-tlog-context">{r.lessons||r.actions||r.notes||"No lessons recorded."}</div>{r.screenshots?.length>0&&<div className="tj-tlog-shots">{r.screenshots.map((src,i)=><img key={i} src={src} alt="review"/>)}</div>}</div>}</Card>}):<div className="tj-empty">No reviews yet.</div>}</div>
  </>;
}
...
function DayTradesModal({ date, trades, account, onClose, onEdit, onDelete }) {
          ...
          return (
            <Card key={t.id} className={`tj-tlog-card tj-tlog-${cls}`}>
              ...
              {t.screenshots?.length > 0 && (
                <div className="tj-tlog-shots" style={{ padding: "0 14px 12px" }}>{t.screenshots.map((src, i) => <img key={i} src={src} alt="" />)}</div>
              )}
            </Card>
          );
        })}
      ...
      <div className="tj-stats-grid">
        ...
        <StatCard label="TRADE WIN %">
          <div className="tj-stat-row">
            ...
            <MultiRing segments={[{ value: stats.wins, color: "#4DFF66" }, { value: stats.be, color: "#60A5FA" }, { value: stats.losses, color: "#F87171" }]} />
          </div>
        ...
        <StatCard label="WIN STREAK">
          <div className="tj-stat-row">
            ...
            <MultiRing segments={[{ value: stats.wins, color: "#4DFF66" }, { value: stats.losses, color: "#F87171" }]} />
          </div>
        </StatCard>
            ...
            <AreaChart data={cumulative}>
              <defs>
                <linearGradient id="cumGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#4DFF66" stopOpacity={0.4} />
                  <stop offset="100%" stopColor="#4DFF66" stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              <Area type="monotone" dataKey="cum" stroke="#4DFF66" fill="url(#cumGrad)" strokeWidth={2.5} dot={{ r: 3, fill: "#4DFF66", strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
          ...
          <ResponsiveContainer width="100%" height={230}>
            <BarChart data={dailyData}>
              ...
              <Bar dataKey="pnl" radius={[4, 4, 4, 4]}>
                {dailyData.map((d, i) => <Cell key={i} fill={d.cls === "loss" ? "#F87171" : d.cls === "be" ? "#60A5FA" : "#4DFF66"} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </Card>
    ...
    <>
      <div className="tj-toolbar">
        ...
        <div className="tj-toolbar-right">
          ...
          <button className={`tj-toolbar-pill ${sortKey === "asset" ? "tj-toolbar-btn-active" : ""}`} onClick={() => toggleSort("asset")}>Asset</button>
        </div>
      </div>
            ...
            return (
              <Card key={t.id} className={`tj-tlog-card tj-tlog-${cls}`}>
                <div className="tj-tlog-row" onClick={() => setExpanded((e) => ({ ...e, [t.id]: !e[t.id] }))}>
                  ...
                  <div className="tj-tlog-types">{t.types.map((tag) => <span key={tag} className="tj-tag tj-tag-purple tj-tag-active tj-tag-xs">{tag}</span>)}</div>
                  <div className="tj-tlog-stars"><RatingDisplay ratingStyle={account.ratingStyle} value={t.rating} /></div>
                  <span className={`tj-statuspill tj-statuspill-${cls}`}>{cls === "win" ? "WIN" : cls === "loss" ? "LOSS" : "B/E"}</span>
...
function AnalyticsPage({ account }) {
  ...
  const dayColor = dayScore >= 86 ? "#4DFF66" : dayScore >= 66 ? "#4DFF66" : dayScore >= 35 ? "#FBBF24" : "#F87171";
    ...
    <div className="tj-analytics">
            ...
            return (
              <div key={t.tag} className="tj-scatter-dot-wrap" style={{ left: `${left}%`, top: `${top}%` }}>
                ...
                <div className={`tj-scatter-tooltip ${flipDown ? "tj-scatter-tooltip-below" : ""}`}>
                  ...
                  <div className="tj-scatter-tooltip-row"><span>Setup</span><span className="tj-bold">{t.tag}</span></div>
                </div>
              </div>
            );
          })}
      ...
      <Card className="tj-panel" style={{ marginTop: 14 }}>
        ...
        <div className="tj-day-dist">
          {greenDays > 0 && <div style={{ width: `${(greenDays / tradingDays.length) * 100}%`, background: "#4DFF66" }} />}
        ...
        <div className="tj-last6-track">
          {last6.map((d, i) => (
            <div key={i} className="tj-last6-bar" style={{ background: d.cls === "loss" ? "#F87171" : d.cls === "be" ? "#60A5FA" : "#4DFF66", flex: Math.max(0.3, Math.abs(d.pnl) / maxAbsLast6) }} title={`${d.date}: ${fmtMoney(d.pnl)}`} />
          ))}
        </div>
...
function CalendarPage({ account, monthCursor, setMonthCursor, onDayClick }) {
  ...
  const heatColor = (cell) => {
    ...
    if (cell.cls === "win") return cell.pnl > 200 ? "#15803D" : "#4DFF66";
          ...
          <div>
              ...
              <div className="tj-cal-grid">
                {weeks.flat().map((cell, i) => (
                  <div key={i}
                    className={`tj-cal-cell tj-dock-cell ${cell ? `tj-cal-${cell.cls || "none"}` : "tj-cal-empty"} ${cell?.iso === todayISO() ? "tj-cal-today" : ""} ${cell && cell.count > 0 ? "tj-cal-clickable" : ""}`}
                    onClick={() => cell && cell.count > 0 && onDayClick(cell.iso)}>
                    {cell && (<><div className="tj-cal-day">{cell.day}</div>{cell.count > 0 && <><div className={`tj-cal-pnl ${clsColor(cell.cls)}`}>{fmtMoneyShort(cell.pnl)}</div><div className="tj-cal-tcount">{cell.count}t</div></>}</>)}
                  </div>
                ))}
              </div>
            </Card>
            ...
            <Card className="tj-panel" style={{ marginTop: 16 }}>
              ...
              <div className="tj-heat-legend">
                ...
                <span className="tj-heat-cell" style={{ background: "#4DFF66" }} />
      ...
      <div className="tj-insight-grid">
        ...
        {bestTag && (
          <div className="tj-insight-card">
            ...
            <div className="tj-muted-txt">Trades tagged "{bestTag.tag}" produce {fmtMoneyShort(bestTag.netPnl)} with <span className={wrColorClass(bestTag.winRate)}>{bestTag.winRate.toFixed(0)}% WR</span>. This is your key level/setup.</div>
          </div>
        )}
        <div className="tj-insight-card">
      ...
      {tab === "impact" && (
        <div>
          ...
          {["high", "medium", "low"].map((lvl) => {
            ...
            const color = lvl === "high" ? "#F87171" : lvl === "medium" ? "#FBBF24" : "#4DFF66";
            return (
...
function RulesPage({ account, onToggleCheckin, onAddRule, onRemoveRule }) {
...
function TradingJournalApp({ user, onLogout }) {
  ...
  const handleCreateMarkup = async (m) => { const res = await createMarkup(user.id, account.id, m); if(res.error) return showError(res.error); setMarkups(x=>[res.data,...x]); setModal(null); };
  ...
  return (
    <div className={`tj-root ${account.theme === "light" ? "tj-theme-light" : ""}`}>
      <style>{CSS}</style>
      ...
      <div className="tj-main">
        ...
        <div className="tj-content">
          ...
          {page === "tradelog" && <TradeLogPage account={account} onNewTrade={() => { setEditingTrade(null); setModal("newtrade"); }} onEdit={(t) => { setEditingTrade(t); setModal("newtrade"); }} onDelete={handleDeleteTrade} />}
          {page === "analytics" && <AnalyticsPage account={account} />}
          {page === "calendar" && <CalendarPage account={account} monthCursor={monthCursor} setMonthCursor={setMonthCursor} onDayClick={setDayModalDate} />}
          {page === "psychology" && <PsychologyPage account={account} />}
          {page === "insights" && <InsightsPage account={account} />}
          {page === "news" && <NewsPage />}
          {page === "rules" && <RulesPage account={account} onToggleCheckin={handleToggleCheckin} onAddRule={handleAddRule} onRemoveRule={handleRemoveRule} />}
          ...
          {page === "markups" && <MarkupsPage markups={markups.filter(m=>m.accountId===account.id)} trades={account.trades} onNew={()=>setModal("markup")} onDelete={handleDeleteMarkup} />}
          {page === "reviews" && <ReviewsPage reviews={reviews.filter(r=>r.accountId===account.id)} trades={account.trades} onSave={handleSaveReview} />}
        </div>
      </div>
      {modal === "newtrade" && <NewTradeModal editing={editingTrade} typeTags={typeTags} mistakeTags={mistakeTags} confluenceSessions={confluenceSessions} markups={markups.filter(m=>m.accountId===account.id)} rules={account.rules} defaultCommission={account.defaultCommission} onAddTypeTag={addTypeTag} ratingStyle={account.ratingStyle} onClose={() => { setModal(null); setEditingTrade(null); }} onSave={saveTrade} />}
      {modal === "editaccount" && <AccountSettingsModal account={account} onClose={() => setModal(null)} onSave={handleSaveAccountSettings} />}
      {modal === "markup" && <MarkupModal onClose={()=>setModal(null)} onSave={handleCreateMarkup} />}
...
.tj-toast-info { background: #12241A; border: 1px solid #4DFF66; color: #4DFF66; }
:root {
  --tj-bg: #0A0B0D; --tj-panel: #131519; --tj-panel-alt: #1B1E24; --tj-border: #262A31;
  --tj-text: #F4F5F7; --tj-muted: #8B8F98; --tj-green: #4DFF66; --tj-red: #F87171;
  --tj-purple: #8B7CF6; --tj-blue: #60A5FA; --tj-amber: #FBBF24;
}
.tj-theme-light {
  --tj-bg: #F4F5F7; --tj-panel: #FFFFFF; --tj-panel-alt: #F1F2F5; --tj-border: #E2E5EA;
  --tj-text: #16181D; --tj-muted: #6B7280; --tj-green: #0F9D63; --tj-red: #DC2626;
...
.tj-logo { height: 72px; display: flex; align-items: center; justify-content: center; border: 1px solid var(--tj-border); border-radius: 10px; background: #08090B; padding: 6px; text-align: center; margin-bottom: 22px; overflow: hidden; }
.tj-logo img { display: block; width: 72px; height: 72px; object-fit: contain; border-radius: 50%; filter: drop-shadow(0 3px 8px rgba(0,0,0,.45)); }
...
.tj-gauge-track { height: 8px; border-radius: 6px; background: linear-gradient(90deg, #F87171, #FBBF24, #4DFF66); position: relative; margin-top: 4px; }
...
.tj-toolbar {
  ...
  background: #111418; border: 1px solid #23272F; border-radius: 12px;
...
.tj-toolbar-search {
  ...
  background: #181C23; border: 1px solid #2A2F38; border-radius: 9px; padding: 0 12px;
  transition: border-color 0.2s ease, box-shadow 0.2s ease;
}
.tj-toolbar-search:focus-within { border-color: #22C55E; box-shadow: 0 0 0 3px rgba(34,197,94,0.18), 0 0 14px rgba(34,197,94,0.25); }
.tj-toolbar-search-icon { color: #6B7280; flex-shrink: 0; }
.tj-toolbar-search-input { background: none; border: none; outline: none; color: #FFFFFF; font-size: 13.5px; font-weight: 500; width: 100%; font-family: inherit; }
.tj-toolbar-search-input::placeholder { color: #6B7280; }
.tj-toolbar-right { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
.tj-toolbar-dd, .tj-toolbar-pill {
  height: 34px; background: #181C23; border: 1px solid #2A2F38; border-radius: 9px;
  color: #E5E7EB; font-size: 13px; font-weight: 500; font-family: 'Inter', system-ui, sans-serif;
  padding: 0 12px; cursor: pointer; white-space: nowrap;
  transition: transform 0.15s ease, border-color 0.15s ease, background 0.15s ease, box-shadow 0.15s ease;
}
.tj-toolbar-dd:hover, .tj-toolbar-pill:hover { transform: translateY(-2px); border-color: #3A4150; }
.tj-toolbar-pill { display: inline-flex; align-items: center; justify-content: center; }
.tj-toolbar-btn-active {
  background: rgba(34,197,94,0.15) !important; border-color: #22C55E !important; color: #22C55E !important;
}
...
.tj-chip-active { border-color: var(--tj-green); color: var(--tj-green); background: rgba(77,255,102,0.12); box-shadow: 0 0 0 1px rgba(77,255,102,0.16); }
...
.tj-scatter-box { position: relative; height: 240px; max-width: 640px; margin: 8px auto 0; border: 1px solid var(--tj-border); border-radius: 10px; }
.tj-scatter-bg { position: absolute; width: 50%; height: 50%; }
.tj-scatter-bg-tl { top: 0; left: 0; background: rgba(52,211,153,0.04); border-top-left-radius: 10px; }
.tj-scatter-bg-tr { top: 0; right: 0; background: rgba(52,211,153,0.14); border-top-right-radius: 10px; }
...
.tj-scatter-quad { position: absolute; font-size: 9px; color: var(--tj-muted); padding: 8px; letter-spacing: 0.3px; font-weight: 700; z-index: 1; }
...
.tj-scatter-dot { border: 2px solid; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 9px; font-weight: 700; background: rgba(0,0,0,0.4); text-align: center; padding: 2px; cursor: pointer; transition: transform 0.15s ease; }
.tj-scatter-dot-wrap:hover .tj-scatter-dot { transform: scale(1.1); }
.tj-scatter-tooltip { position: absolute; bottom: calc(100% + 10px); left: 50%; transform: translateX(-50%); background: var(--tj-panel-alt); border: 1px solid var(--tj-border); border-radius: 10px; padding: 10px 12px; width: 160px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); opacity: 0; pointer-events: none; transition: opacity 0.15s ease; z-index: 10; }
...
.tj-daytag-today { background: rgba(52,211,153,0.18); color: var(--tj-green); }
...
.tj-impactpill-low { background: rgba(52,211,153,0.15); color: var(--tj-green); }
