...
function NewTradeModal({ onClose, onSave, editing, typeTags, mistakeTags, confluenceSessions, markups, rules, defaultCommission }) {
  ...
  return <Modal title={editing ? "Edit Trade" : "New Trade"} onClose={onClose} wide>
    ...
    <div className="tj-grid4">
      <Field label="Asset *"><input className="tj-input" placeholder="Type or select..." value={form.asset} onChange={(event) => set("asset", event.target.value)} list="tj-asset-list" /><datalist id="tj-asset-list">{ASSET_LIST.map((asset) => <option key={asset} value={asset} />)}</datalist></Field>
...
function MarkupModal({ onClose, onSave, editing }) {
  ...
  return <Modal title={editing ? "Edit Premarket Markup" : "New Premarket Markup"} onClose={onClose} wide>
    <div className="tj-grid4"><Field label="Date"><input type="date" className="tj-input" value={f.date} onChange={e=>set("date",e.target.value)}/></Field><Field label="Time *"><input type="time" required className="tj-input" value={f.time} onChange={e=>set("time",e.target.value)}/></Field><Field label="Market / Session"><input className="tj-input" placeholder="NY AM" value={f.market} onChange={e=>set("market",e.target.value)}/></Field><Field label="Instrument"><input className="tj-input" placeholder="EUR/USD" value={f.instrument} onChange={e=>set("instrument",e.target.value)}/></Field></div>
...
function CalendarPage({ account, markups = [], reviews = [], monthCursor, setMonthCursor, onDayClick }) {
  ...
  const recordsByDate = useMemo(() => {
    const records = {};
    const add = (record) => { if (!record.date) return; (records[record.date] ||= []).push(record); };
    account.trades.forEach((trade) => add({ id: `trade-${trade.id}`, date: trade.date, time: trade.time, kind: "Trade", label: `${trade.asset} ${trade.direction}`, pnl: trade.pnl }));
    markups.forEach((markup) => add({ id: `markup-${markup.id}`, date: markup.date, time: markup.time, kind: "Markup", label: markup.instrument || markup.market || "Premarket markup" }));
    reviews.forEach((review) => { const trade = account.trades.find((item) => item.id === review.tradeId); add({ id: `review-${review.id}`, date: review.date, time: review.time, kind: "Review", label: trade ? `${trade.asset} review` : "Trade review" }); });
    Object.values(records).forEach((items) => items.sort((a, b) => String(a.time || "99:99").localeCompare(String(b.time || "99:99"))));
    return records;
  }, [account.trades, markups, reviews]);
          ...
          <div>
            <Card className="tj-panel">
              ...
              <div className="tj-cal-grid">
                {weeks.flat().map((cell, i) => (
                  <div key={i}
                    className={`tj-cal-cell tj-dock-cell ${cell ? `tj-cal-${cell.cls || "none"}` : "tj-cal-empty"} ${cell?.iso === todayISO() ? "tj-cal-today" : ""} ${cell && (cell.count > 0 || recordsByDate[cell.iso]?.length) ? "tj-cal-clickable" : ""}`}
                    onClick={() => cell && recordsByDate[cell.iso]?.length && onDayClick(cell.iso)}>
                    {cell && (<><div className="tj-cal-day">{cell.day}</div>{cell.count > 0 && <><div className={`tj-cal-pnl ${clsColor(cell.cls)}`}>{fmtMoneyShort(cell.pnl)}</div><div className="tj-cal-tcount">{cell.count}t</div></>}{recordsByDate[cell.iso]?.slice(0, 2).map((record) => <div className="tj-cal-record" key={record.id}><span>{formatTime(record.time)}</span><span>{record.label}</span></div>)}{(recordsByDate[cell.iso]?.length || 0) > 2 && <div className="tj-cal-more">+{recordsByDate[cell.iso].length - 2} more</div>}</>) }
                  </div>
                ))}
              </div>
            </Card>
...
:root {
  ...
  --tj-text: #F4F5F7; --tj-muted: #8B8F98; --tj-green: #22C55E; --tj-red: #F87171;
  --tj-purple: #8B7CF6; --tj-blue: #60A5FA; --tj-amber: #FBBF24;
}
.tj-theme-light {
  --tj-bg: #F4F5F7; --tj-panel: #FFFFFF; --tj-panel-alt: #F1F2F5; --tj-border: #E2E5EA;
  --tj-text: #16181D; --tj-muted: #6B7280; --tj-green: #22C55E; --tj-red: #DC2626;
...
.tj-chip-active { border-color: var(--tj-green); color: var(--tj-green); background: rgba(34,197,94,0.12); box-shadow: 0 0 0 1px rgba(34,197,94,0.16); }
...
.tj-scatter-box { position: relative; height: 440px; max-width: none; margin: 8px 0 22px; border: 1px solid var(--tj-border); border-radius: 10px; }
