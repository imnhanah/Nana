...
function AccountSettingsModal({ account, onClose, onSave }) {
  ...
  const [open, setOpen] = useState({ identity: true, defaults: true, appearance: true, goals: true, guardrails: true });
  ...
  const Section = ({ id, title, children, note }) => <section className="tj-settings-section">
    <button type="button" className="tj-settings-section-head" onClick={() => toggle(id)} aria-expanded={open[id]}>
      <span><strong>{title}</strong>{note && <small>{note}</small>}</span><ChevronDown size={16} style={{ transform: open[id] ? "rotate(180deg)" : "none" }} />
    </button>
  ...
  return (
    <Modal title={<span><Settings size={16} style={{ marginRight: 6, verticalAlign: -3 }} />Account Settings</span>} onClose={onClose} wide>
      <div className="tj-settings-summary"><span className="tj-settings-summary-kicker">ACCOUNT CONTROL CENTER</span><span>All goals and loss caps are optional. Set a value above 0 to activate it; leave it at 0 to keep that guardrail off.</span></div>
      <Section id="identity" title="Identity & Profile" note="Personalize this trading account.">
      ...
      <Section id="defaults" title="Journal Defaults" note="Used as the starting point for new trades.">
      ...
      <Section id="appearance" title="Appearance" note="Apply the theme used across this account.">
        <div className="tj-field-label">Theme</div><div className="tj-chip-row tj-theme-choice-row"><button type="button" aria-pressed={theme === "dark"} className={`tj-chip-big tj-theme-choice ${theme === "dark" ? "tj-chip-active" : ""}`} onClick={() => setTheme("dark")}>DARK</button><button type="button" aria-pressed={theme === "light"} className={`tj-chip-big tj-theme-choice ${theme === "light" ? "tj-chip-active" : ""}`} onClick={() => setTheme("light")}>LIGHT</button></div>
      </Section>
      <Section id="goals" title="Goals" note="Optional performance targets for this account.">
      ...
      <Section id="guardrails" title="Risk Guardrails" note="Positive caps lock new trade entry once reached; markups stay available.">
...
function ReferenceDashboardPage({ account, stats, monthCursor, setMonthCursor, onDayClick, guardrails }) {
  ...
  const thisYearTrades = account.trades.filter((trade) => trade.date?.slice(0, 4) === String(new Date().getFullYear()));
  const thisYearStats = computeStats(thisYearTrades, account.breakevenCap);
  ...
  const summaryCards = [
    ["NET P&L", fmtMoney(thisYearStats.netPnl), thisYearStats.netPnl >= 0 ? "tj-green" : "tj-red", `${thisYearStats.total} trades this year`],
    ["PROFIT FACTOR", thisYearStats.profitFactor.toFixed(2), "", `${fmtMoney(thisYearStats.grossProfit)} gross profit`],
    ["DAY WIN %", `${thisYearStats.dayWinRate.toFixed(0)}%`, wrColorClass(thisYearStats.dayWinRate), `${thisYearStats.dayClasses.length} trading days`],
    ["WIN RATE", `${thisYearStats.winRate.toFixed(0)}%`, wrColorClass(thisYearStats.winRate), `${thisYearStats.wins}W · ${thisYearStats.losses}L · ${thisYearStats.be} B/E`],
    ["AVG WIN / LOSS", thisYearStats.avgLoss ? thisYearStats.avgWinLoss.toFixed(2) : "—", "", `${fmtMoney(thisYearStats.avgWin)} / ${fmtMoney(-thisYearStats.avgLoss)}`],
  ];
  ...
  return <div className="tj-reference-dashboard">
    ...
    <div className="tj-reference-kpis">{summaryCards.map(([label, value, tone, note]) => <Card key={label} className="tj-reference-kpi"><div className="tj-stat-label">{label}</div><div className={`tj-reference-kpi-value ${tone}`}>{value}</div><div className="tj-stat-sub">{note}</div></Card>)}</div>

    <div className="tj-reference-calendar-layout">
      <Card className="tj-panel tj-reference-calendar">
        <div className="tj-panel-head"><span>{MONTH_NAMES[month]} {year}</span><div className="tj-month-nav"><button className="tj-icon-btn" onClick={() => setMonthCursor(new Date(year, month - 1, 1))}><ChevronLeft size={16}/></button><button className="tj-btn-outline tj-btn-small" onClick={() => setMonthCursor(new Date())}>This month</button><button className="tj-icon-btn" onClick={() => setMonthCursor(new Date(year, month + 1, 1))}><ChevronRight size={16}/></button></div></div>
        <div className="tj-month-summary"><div><div className="tj-green tj-mnum">{monthStats.wins}</div><div className="tj-mlabel">PROFIT</div></div><div><div className="tj-red tj-mnum">{monthStats.losses}</div><div className="tj-mlabel">LOSS</div></div><div><div className="tj-blue tj-mnum">{monthStats.be}</div><div className="tj-mlabel">B/E</div></div><div><div className="tj-mnum">{monthStats.total}</div><div className="tj-mlabel">TRADES</div></div></div>
        <div className="tj-cal-dow">{DOW.map((day) => <div key={day}>{day}</div>)}</div>
        <div className="tj-cal-grid">{weeks.flat().map((cell, index) => <div key={index} className={`tj-cal-cell tj-dock-cell ${cell ? `tj-cal-${cell.cls || "none"}` : "tj-cal-empty"} ${cell?.iso === todayISO() ? "tj-cal-today" : ""} ${cell?.count ? "tj-cal-clickable" : ""}`} onClick={() => cell?.count && onDayClick(cell.iso)}>{cell && <><div className="tj-cal-day">{cell.day}</div>{cell.count > 0 && <><div className={`tj-cal-pnl ${clsColor(cell.cls)}`}>{fmtMoneyShort(cell.pnl)}</div><div className="tj-cal-tcount">{cell.count} trade{cell.count === 1 ? "" : "s"}</div></>}</>}</div>)}</div>
      </Card>
      <Card className="tj-panel tj-reference-weeks"><div className="tj-panel-head"><span>Weekly Snapshot</span></div>{weeklyBreakdown.length ? <div className="tj-weekly-list">{weeklyBreakdown.map((week) => <div key={week.label} className="tj-weekly-item"><div className="tj-weekly-item-label">{week.label}</div><div className={`tj-weekly-item-num ${week.pnl >= 0 ? "tj-green" : "tj-red"}`}>{fmtMoney(week.pnl)}</div><div className="tj-weekly-item-sub">{week.count} trades · <span className={wrColorClass(week.winRate)}>{week.winRate.toFixed(0)}% WR</span></div><div className="tj-bar-track"><div className={`tj-bar-fill ${wrBarClass(week.winRate)}`} style={{ width: `${week.winRate}%` }} /></div></div>)}</div> : <div className="tj-empty">No trades this month.</div>}</Card>
    </div>

    <Card className="tj-panel tj-reference-flow">
      <div className="tj-panel-head"><div className="tj-tabs"><button className={`tj-tab ${view === "recent" ? "tj-tab-active" : ""}`} onClick={() => setView("recent")}>Recent Trades</button><button className={`tj-tab ${view === "flow" ? "tj-tab-active" : ""}`} onClick={() => setView("flow")}>Performance Flow</button></div><span className="tj-muted-txt">Net P&amp;L is the source of truth</span></div>
      {view === "recent" ? <div className="tj-recent-table"><div className="tj-recent-head"><span>Close Date</span><span>Instrument</span><span>Side</span><span>Session</span><span>Net P&amp;L</span></div>{recentTrades.length ? recentTrades.map((trade) => <div className="tj-recent-row" key={trade.id}><span>{trade.date} · {formatTime(trade.time)}</span><strong>{trade.asset}</strong><span className={trade.direction === "BUY" ? "tj-green" : "tj-red"}>{trade.direction}</span><span>{trade.entrySession || trade.session || "—"}</span><strong className={trade.pnl >= 0 ? "tj-green" : "tj-red"}>{fmtMoney(trade.pnl)}</strong></div>) : <div className="tj-empty">No trades yet. Your newest completed trade will appear here.</div>}</div> : <div className="tj-flow-grid"><div className="tj-flow-last"><div className="tj-field-label">LAST 6 CLOSES</div><div className="tj-last6-track">{lastSix.length ? lastSix.map((trade) => <div key={trade.id} className={`tj-last6-bar ${trade.pnl >= 0 ? "tj-bar-green" : "tj-bar-red"}`} style={{ flex: Math.max(1, Math.abs(trade.pnl)) }} title={`${trade.asset}: ${fmtMoney(trade.pnl)}`} />) : <div className="tj-empty">Complete a trade to start your performance flow.</div>}</div><div className="tj-last6-dates">{lastSix.map((trade) => <span key={trade.id}>{trade.date.slice(5)}</span>)}</div></div><div className="tj-flow-metric"><span>ACCOUNT RETURN</span><strong className={flowPct >= 0 ? "tj-green" : "tj-red"}>{flowPct >= 0 ? "+" : ""}{flowPct.toFixed(2)}%</strong><small>{fmtMoney(stats.netPnl)} net</small></div><div className="tj-flow-metric"><span>BEST SESSION</span><strong className={bestSession?.pnl >= 0 ? "tj-green" : "tj-red"}>{bestSession?.session || "—"}</strong><small>{bestSession ? fmtMoney(bestSession.pnl) : "No trades"}</small></div><div className="tj-flow-metric"><span>HOT INSTRUMENT</span><strong>{hotPair?.[0] || "—"}</strong><small className={hotPair?.[1] >= 0 ? "tj-green" : "tj-red"}>{hotPair ? fmtMoney(hotPair[1]) : "No trades"}</small></div><div className="tj-flow-metric"><span>CURRENT STREAK</span><strong className={stats.streakType === "loss" ? "tj-red" : "tj-green"}>{stats.streak ? `${stats.streak}${stats.streakType === "loss" ? "L" : "W"}` : "—"}</strong><small>{stats.bestWinStreak}W best run</small></div></div>}
    </Card>
  </div>;
}
...
function TradingJournalApp({ user, onLogout }) {
  ...
  const openNewTrade = () => {
    ...
    setEditingTrade(null); setModal("newtrade");
  };
    ...
    <div className={`tj-root ${account.theme === "light" ? "tj-theme-light" : "tj-theme-dark"}`}>
      ...
      <div className="tj-main">
        ...
        <div className="tj-content">
          ...
          {page === "tradelog" && <TradeLogPage account={account} reviews={reviews.filter((review) => review.accountId === account.id)} onNewTrade={openNewTrade} onEdit={(t) => { setEditingTrade(t); setModal("newtrade"); }} onDelete={handleDeleteTrade} />}
...
.tj-settings-section-head { width: 100%; display: flex; align-items: center; justify-content: space-between; gap: 12px; border: none; background: none; color: var(--tj-text); padding: 12px 14px; text-align: left; cursor: pointer; font: inherit; }
.tj-settings-section-head span { display: grid; gap: 2px; }.tj-settings-section-head small { color: var(--tj-muted); font-size: 11px; font-weight: 400; }.tj-settings-section-head svg { transition: transform .16s ease; color: var(--tj-muted); }
...
.tj-reference-kpis { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 10px; }.tj-reference-kpi { padding: 14px; min-height: 94px; }.tj-reference-kpi-value { font-size: 20px; font-weight: 800; margin: 7px 0 5px; font-variant-numeric: tabular-nums; letter-spacing: -.25px; }.tj-reference-kpi .tj-stat-sub { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.tj-reference-calendar-layout { display: grid; grid-template-columns: minmax(0, 1.75fr) minmax(260px, .75fr); gap: 14px; }.tj-reference-calendar, .tj-reference-weeks, .tj-reference-flow { padding: 16px; }.tj-reference-calendar .tj-month-summary { margin-bottom: 14px; }.tj-reference-calendar .tj-cal-cell { min-height: 78px; }.tj-reference-calendar .tj-cal-tcount { text-transform: none; font-size: 9px; margin-top: 2px; }.tj-reference-weeks .tj-weekly-list { max-height: 385px; overflow: auto; padding-right: 2px; }
.tj-reference-flow .tj-panel-head { margin-bottom: 12px; }.tj-recent-table { display: grid; gap: 2px; }.tj-recent-head, .tj-recent-row { display: grid; grid-template-columns: 1.35fr 1fr .65fr .85fr .8fr; align-items: center; gap: 12px; }.tj-recent-head { color: var(--tj-muted); font-size: 10px; letter-spacing: .55px; font-weight: 700; text-transform: uppercase; padding: 0 11px 8px; }.tj-recent-row { min-height: 44px; padding: 8px 11px; border: 1px solid var(--tj-border); background: var(--tj-panel-alt); border-radius: 8px; font-size: 12px; }.tj-recent-row strong:last-child { text-align: right; font-variant-numeric: tabular-nums; }
.tj-flow-grid { display: grid; grid-template-columns: minmax(230px, 1.55fr) repeat(4, minmax(130px, 1fr)); gap: 10px; }.tj-flow-last, .tj-flow-metric { border: 1px solid var(--tj-border); background: var(--tj-panel-alt); border-radius: 9px; padding: 12px; min-width: 0; }.tj-flow-last .tj-last6-track { min-height: 38px; }.tj-flow-metric { display: grid; align-content: center; gap: 4px; }.tj-flow-metric span { color: var(--tj-muted); font-size: 10px; letter-spacing: .55px; font-weight: 700; }.tj-flow-metric strong { font-size: 16px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.tj-flow-metric small { color: var(--tj-muted); font-size: 11px; }
...
@media (max-width: 900px) {
  ...
  .tj-reference-kpis { grid-template-columns: repeat(3, minmax(0, 1fr)); }.tj-guardrail-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }.tj-reference-calendar-layout { grid-template-columns: 1fr; }.tj-flow-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }.tj-flow-last { grid-column: 1 / -1; }
}
