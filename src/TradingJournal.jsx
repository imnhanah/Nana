  ...
  return (
    <Modal title={<span className="tj-symbol-title"><NavSymbol src={accountSettingsSymbol} />Account Settings</span>} onClose={onClose} onConfirm={saveSettings} confirmDisabled={!name.trim() || !baseCurrency} wide>
      ...
      <AccountSettingsSection title="Challenge" icon={<Trophy size={15}/>} note="Build a compounding plan from your own starting amount." status={challengeEnabled && Number(challengeStartingBalance) > 0 ? "On" : "Off"} isOpen={open.challenge} onToggle={() => toggle("challenge")}>
        ...
        <p className="tj-muted-txt">The starting amount records your deposit. The planned ladder and final target stay fixed. Turning Challenge off preserves your level and notes and returns the challenge to manual mode. This does not change your trading account balance.</p>
        <ChallengeModeSettings challenge={challenge} enabled={isChallengeEnabled(account) && challengeEnabled}/>
      </AccountSettingsSection>
...
function TradingJournalApp({ user, onLogout }) {
  ...
  const handleSaveAccountSettings = async (updated) => {
    if (isChallengeEnabled(account) && !isChallengeEnabled(updated) && challenge.state.automation?.mode) {
      if (!(await challenge.setMode(null))) return;
    }
