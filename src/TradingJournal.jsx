...
import { ThemedFields, ThemeSelect, ThemeTime, ThemeDate, DeleteConfirmation, confirmDelete } from './JournalControls';
            ...
            {screenshots.map((src, i) => (
              <div key={i} className="tj-shot-thumb">
                ...
                <button type="button" className="tj-shot-remove"
                  aria-label="Remove screenshot" onClick={async (e) => { e.stopPropagation(); if (await confirmDelete('Remove this screenshot? Save the form to keep this change.')) onChange((prev) => prev.filter((_, idx) => idx !== i)); }}>
                  <X size={12} />
                </button>
              </div>
            ))}
...
function NewTradeModal({ onClose, onSave, editing, draft, typeTags, mistakeTags, confluenceSessions, markups, rules, instruments = [], defaultCommission, account }) {
  ...
  return <Modal title={editing ? "Edit Trade" : "Log Trade"} onClose={onClose} onConfirm={save} confirmDisabled={!form.asset.trim() || form.grossPnl === "" || !form.time || !!timingError} wide>
    ...
    <div className="tj-grid2"><Field label="Open date *"><input type="date" value={form.date} onChange={event=>set('date',event.target.value)}/></Field><Field label="Open time *"><input type="time" value={form.time} onChange={event=>set('time',event.target.value)}/></Field></div>
    <div className="tj-grid2 tj-trade-timing"><Field label="Close date (optional)"><input type="date" value={form.closeDate || ''} onChange={event=>set('closeDate',event.target.value)}/></Field><Field label="Close time (optional)"><input type="time" value={form.closeTime || ''} onChange={event=>set('closeTime',event.target.value)}/></Field></div>
    ...
    <Modal title={<span className="tj-symbol-title"><NavSymbol src={profileSettingsSymbol} />Profile</span>} onClose={onClose} onConfirm={save} confirmDisabled={saving || !fullName.trim()} wide>
      <div className="tj-personal-profile-card">
        ...
        <div className="tj-personal-profile-photo-row">
          ...
          <div><strong>{fullName.trim() || "Your name"}</strong><div className="tj-chip-row"><button type="button" className="tj-btn-outline tj-btn-small" onClick={() => photoRef.current?.click()}>Change photo</button>{profileImage && <button type="button" className="tj-btn-outline tj-btn-small" onClick={async () => { if (await confirmDelete("Remove this profile photo? Save the profile to keep this change.")) setProfileImage(""); }}>Remove photo</button>}</div></div>
...
function ListManager({ title, items, onSave, note }) {
  ...
  const remove = async (v) => { if (await confirmDelete(`Remove “${v}” from future trade entry? Historical trades retain their saved value.`)) onSave(items.filter((x) => x !== v)); };
              ...
              <Card key={t.id} className={`tj-tlog-card tj-reference-tradelog-row tj-tlog-${cls}`}>
                <div className="tj-tlog-row" onClick={() => setExpanded((e) => ({ ...e, [t.id]: !e[t.id] }))}>
                  <div className="tj-reference-trade-left">
                    ...
                    <div className="tj-reference-trade-meta"><span>Opened: {new Date(t.date + "T00:00:00").toLocaleDateString(undefined, { month: "short", day: "numeric", year: "2-digit" })} · {formatTime(t.time)}</span><span>Closed: {t.closeDate ? `${t.closeDate} · ${t.closeTime || "Time not logged"}` : "Not logged"}</span><span>{t.entrySession || t.session || "No session"}</span></div>
...
function TradingJournalApp({ user, onLogout }) {
  ...
  const handleDeleteTrade = async (id) => {
    if (!(await confirmDelete('Delete this trade? This cannot be undone.'))) return;
  ...
  const handleDeleteMarkup = async (id) => { if(!await confirmDelete("Delete this markup? Linked trades will retain their historical reference as empty.")) return; const res=await deleteMarkup(id); if(res.error)return showError(res.error); setMarkups(x=>x.filter(m=>m.id!==id)); };
  ...
  const handleResetData = async () => {
    if (!await confirmDelete("Delete every trade for this account and reset it to its starting balance? This can't be undone.")) return;
  ...
  const handleDeleteAccount = async (a) => {
    if (!await confirmDelete(`Permanently delete "${a.name}" and all of its trades, markups, reviews, and history? This cannot be undone.`)) return false;
  ...
  const handleRemoveRule = async (id) => {
    if (!(await confirmDelete('Delete this rule from this account?'))) return;
    ...
    <div className={`tj-root journal-dashboard-arrive ${profileTheme === "light" ? "tj-theme-light" : "tj-theme-dark"}`}>
      ...
      <div className="tj-main">
        ...
        <div className="tj-content">
          ...
          {page === "challenge" && isChallengeEnabled(account) && <ChallengePage key={account.id} account={account} userId={user.id} loginQuote={loginQuote}/>}
