...
function Modal({ title, onClose, children, wide, onConfirm, confirmDisabled = false, className = "" }) {
  return (
...
function InstrumentPicker({ value, onChange }) {
  ...
  const compact = (text) => text.replace(/[^a-z0-9]/gi, "").toUpperCase();
  const options = DEFAULT_INSTRUMENTS.filter((item) => compact(item).includes(compact(query)));
  const displayValue = DEFAULT_INSTRUMENTS.find((item) => compact(item) === compact(value || "")) || value || "";
  const select = (item) => { onChange(item); setOpen(false); setQuery(""); };
  const move = (event) => {
    if (event.key === "Escape") { event.stopPropagation(); setOpen(false); }
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      event.preventDefault();
      if (!open) { setOpen(true); setQuery(""); setIndex(0); return; }
      const next = Math.max(0, Math.min(options.length - 1, index + (event.key === "ArrowDown" ? 1 : -1)));
      setIndex(next);
      document.getElementById(`${listId}-${next}`)?.scrollIntoView({ block: "nearest" });
    }
    if (event.key === "Enter" && open) { event.preventDefault(); if (options[index]) select(options[index]); }
  };
  return <div className="tj-instrument-picker" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false); }}>
    <div className="tj-instrument-control">
      <input className="tj-input" role="combobox" aria-label="Instrument" aria-autocomplete="list" aria-expanded={open} aria-controls={listId} aria-activedescendant={open && options[index] ? `${listId}-${index}` : undefined} autoComplete="off" placeholder="Select an instrument..." value={open ? query : displayValue} onFocus={() => { setOpen(true); setQuery(""); setIndex(0); }} onClick={() => setOpen(true)} onChange={(event) => { setQuery(event.target.value); setIndex(0); setOpen(true); }} onKeyDown={move} />
      <ChevronDown size={14} aria-hidden="true" />
    </div>
    {open && <div id={listId} role="listbox" aria-label="Instruments" className="tj-instrument-options">{options.map((item, i) => <button type="button" role="option" id={`${listId}-${i}`} key={item} aria-selected={item === displayValue} className={i === index ? "tj-instrument-option-focused" : ""} onMouseDown={(event) => event.preventDefault()} onClick={() => select(item)}>{item}</button>)}{!options.length && <div className="tj-instrument-empty">No matching instruments</div>}</div>}
  </div>;
}
...
function NewTradeModal({ onClose, onSave, editing, draft, typeTags, mistakeTags, confluenceSessions, markups, rules, instruments = [], defaultCommission, account }) {
  const [form, setForm] = useState(() => {
    ...
    return { ...base, ...source, time: source.time || nowTime(), entryType: source.entryType || source.confluenceSession || "", entrySession: normalizeSession(source.entrySession || source.session || SESSIONS[2]), confluence: source.confluence || source.types || [], ruleEvaluations: source.ruleEvaluations || [] };
  });
  ...
  return <Modal title={editing ? "Edit Trade" : "Log Trade"} onClose={onClose} onConfirm={save} confirmDisabled={!form.asset.trim() || form.grossPnl === "" || !form.time} wide>
    ...
    <div className={account?.positionSizeEnabled ? "tj-grid2" : "tj-grid4 tj-trade-identity-grid"}>
      ...
      {!account?.positionSizeEnabled && <><Field label="Date"><input type="date" className="tj-input" value={form.date} onChange={(event) => set("date", event.target.value)} /></Field><Field label="Time *"><input type="time" required className="tj-input" value={form.time} onChange={(event) => set("time", event.target.value)} /></Field></>}
    </div>
    {account?.positionSizeEnabled && <>
      ...
      <div className="tj-grid2"><Field label="Date"><input type="date" className="tj-input" value={form.date} onChange={(event) => set("date", event.target.value)} /></Field><Field label="Time *"><input type="time" required className="tj-input" value={form.time} onChange={(event) => set("time", event.target.value)} /></Field></div>
    </>}
...
function AccountSettingsModal({ account, onClose, onSave, onDelete }) {
  ...
  return (
    <Modal title={<span className="tj-symbol-title"><NavSymbol src={accountSettingsSymbol} />Account Settings</span>} onClose={onClose} onConfirm={saveSettings} confirmDisabled={!name.trim() || !baseCurrency} wide>
      ...
      <AccountSettingsSection title="Challenge" icon={<Trophy size={15}/>} note="Build a compounding plan from your own starting amount." status={challengeEnabled && Number(challengeStartingBalance) > 0 ? "On" : "Off"} isOpen={open.challenge} onToggle={() => toggle("challenge")}>
        ...
        <p className="tj-muted-txt">The starting amount records your deposit. The planned ladder and final target stay fixed. Turning Challenge off preserves your saved levels and notes. This does not change your trading account balance.</p>
      </AccountSettingsSection>
      ...
      <AccountSettingsSection title="Danger Zone" icon={<Trash2 size={15}/>} danger isOpen={open.danger} onToggle={() => toggle("danger")}>
        <div className="tj-settings-danger-action"><span><strong>Delete Account</strong><small>This permanently deletes this account and all of its trades, markups, reviews, and history.</small></span><button type="button" className="tj-btn-danger-outline" onClick={() => onDelete(account)}><Trash2 size={14}/> Delete Account</button></div>
      </AccountSettingsSection>
      <div className="tj-modal-actions">
        <button className="tj-btn-outline" onClick={onClose}>Cancel</button>
        <button className="tj-btn-primary" disabled={!name.trim() || !baseCurrency} title={!baseCurrency ? "Choose an account base currency before saving" : ""} onClick={saveSettings}>Save Settings</button>
      </div>
    </Modal>
  );
}
    ...
    <Modal title={<span className="tj-symbol-title"><NavSymbol src={profileSettingsSymbol} />Profile</span>} onClose={onClose} onConfirm={save} confirmDisabled={saving || !fullName.trim()} wide>
      <div className="tj-personal-profile-card">
        ...
        <div className="tj-personal-profile-photo-row">
          ...
          <div><strong>{fullName.trim() || "Your name"}</strong><div className="tj-chip-row"><button type="button" className="tj-btn-outline tj-btn-small" onClick={() => photoRef.current?.click()}>Change photo</button>{profileImage && <button type="button" className="tj-btn-outline tj-btn-small" onClick={() => setProfileImage("")}>Remove photo</button>}</div></div>
...
function AddAccountModal({ onClose, onCreate }) {
  ...
  return (
    <Modal title="New Account Workspace" onClose={onClose} onConfirm={create} confirmDisabled={!name.trim() || !baseCurrency || creating} className="tj-new-account-modal" wide>
      <div className="tj-settings-account-hero tj-new-account-hero">
        <div className="tj-settings-hero-avatar" aria-hidden="true"><span>{accountInitial(name || "Untitled Account")}</span></div>
...
function DayTradesModal({ date, trades, reviews = [], account, onClose, onEdit, onDelete }) {
  ...
  return (
    <Modal title={d.toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" })} onClose={onClose} className="tj-calendar-day-modal">
          ...
          return <div key={t.id} className={`tj-day-trade tj-day-trade-${cls}`}>
            ...
            <div className="tj-day-trade-footer">
              ...
              <div className="tj-day-trade-actions">
                ...
                <button type="button" className="tj-icon-btn tj-day-trade-delete" title="Delete trade" aria-label="Delete trade" onClick={() => onDelete(t.id)}><Trash2 size={14}/></button>
              </div>
            </div>
          </div>;
        })}
...
function ReferenceDashboardPage({ account, stats, monthCursor, setMonthCursor, onDayClick, guardrails, displayName, loginQuote }) {
  ...
  return <div className="tj-reference-dashboard">
    <section className="tj-dashboard-welcome" aria-label="Dashboard welcome">
      <div className="tj-dashboard-welcome-copy"><span>AAICOREFX / DASHBOARD</span><h1>Welcome back, {displayName}!</h1><p>Review the month at a glance, then use the record to decide what deserves a closer look.</p></div>
...
function TradeLogPage({ account, reviews = [], markups = [], onEdit, onDelete, onNewTrade, onLinkMarkup }) {
  ...
  return (
    <>
      ...
      <div className="tj-tradelog-compact-toolbar">
              ...
              <Card key={t.id} className={`tj-tlog-card tj-reference-tradelog-row tj-tlog-${cls}`}>
                <div className="tj-tlog-row" onClick={() => setExpanded((e) => ({ ...e, [t.id]: !e[t.id] }))}>
                  <div className="tj-reference-trade-left">
                    ...
                    <div className="tj-reference-trade-meta"><span>{new Date(t.date + "T00:00:00").toLocaleDateString(undefined, { month: "short", day: "numeric", year: "2-digit" })} · {formatTime(t.time)}</span><span>{t.entrySession || t.session || "No session"}</span></div>
                  ...
                  <div className="tj-reference-trade-right" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button className="tj-markup-round-button tj-markup-delete-button" title="Delete trade" onClick={() => onDelete(t.id)}><Trash2 size={14}/></button>
...
function RiskManagementInsightsView({ account, trades, stats }) {
  ...
  const winningHolds = trades.filter((trade) => trade.pnl > 0 && trade.openTime && trade.exitTime);
  const losingHolds = trades.filter((trade) => trade.pnl < 0 && trade.openTime && trade.exitTime);
  const averageHold = (rows) => rows.length ? rows.reduce((sum, trade) => {
    const start = new Date(`${trade.date}T${trade.openTime}`);
    const end = new Date(`${trade.exitDate || trade.date}T${trade.exitTime}`);
...
function TradingJournalApp({ user, onLogout }) {
  ...
  const handleDeleteAccount = async (a) => {
    if (!window.confirm(`Permanently delete "${a.name}" and all of its trades, markups, reviews, and history? This cannot be undone.`)) return false;
  ...
  return (
    ...
    <div className={`tj-root ${profileTheme === "light" ? "tj-theme-light" : "tj-theme-dark"}`}>
      <style>{CSS}</style>
            ...
            <div className="tj-account-menu">
              ...
              <div className="tj-account-center-list">{accounts.map((a) => (
                <div key={a.id} className={`tj-account-row-wrap ${a.id === activeId ? "tj-account-row-active" : ""}`}>
                  ...
                  {accounts.length > 1 && <button className="tj-account-delete" title="Delete account" onClick={(e) => { e.stopPropagation(); handleDeleteAccount(a); }}><Trash2 size={13} /></button>}
                </div>
              ))}</div>
      ...
      <div className="tj-main">
        ...
        <div className="tj-content">
          ...
          {page === "challenge" && isChallengeEnabled(account) && <ChallengePage key={account.id} account={account} userId={user.id} />}
      ...
      {modal === "account" && <AccountSettingsModal account={account} onClose={() => setModal(null)} onSave={handleSaveAccountSettings} onDelete={handleDeleteAccount} />}
...
.tj-root { font-family: 'Inter', system-ui, -apple-system, sans-serif; background-color: var(--tj-bg); background-image: radial-gradient(circle at 7% 8%, var(--tj-bg-glow-left), transparent 32%), radial-gradient(circle at 94% 7%, var(--tj-bg-glow-right), transparent 40%), linear-gradient(var(--tj-grid-line) 1px, transparent 1px), linear-gradient(90deg, var(--tj-grid-line) 1px, transparent 1px); background-size: auto, auto, 32px 32px, 32px 32px; background-attachment: fixed; color: var(--tj-text); color-scheme: dark; display: flex; height: 100vh; width: 100%; font-size: 1rem; overflow: hidden; }
...
.tj-dirpill-sm { font-size: 0.75rem; font-weight: 700; background: var(--tj-panel-alt); border-radius: 4px; padding: 1px 5px; }
...
.tj-theme-nav { width: 38px; min-height: 38px; justify-content: center; margin: 7px 0 0 4px; padding: 0; border-color: color-mix(in srgb, var(--tj-border) 78%, transparent); border-radius: 10px; background: color-mix(in srgb, var(--tj-panel-alt) 72%, transparent); }.tj-theme-nav svg { flex: 0 0 auto; color: var(--tj-amber); }.tj-theme-nav:hover { border-color: color-mix(in srgb, var(--tj-amber) 42%, var(--tj-border)); background: color-mix(in srgb, var(--tj-amber) 9%, var(--tj-panel-alt)); color: var(--tj-amber); }.tj-theme-light .tj-theme-nav svg { color: var(--tj-purple); }
...
.tj-dashboard-welcome h1 { font-size: clamp(1.7rem, 3vw, 2.8rem); line-height: 1.15; letter-spacing: -1.2px; margin: 12px 0 14px; overflow-wrap: anywhere; }
...
export default function AppRoot() {
  ...
  if (user === undefined) {
    return (
      <div className="tj-root tj-loading">
        <style>{`.tj-root{font-family:'Inter',system-ui,sans-serif;background:#0B1016;color:#95A1B1;} .tj-spinner{width:28px;height:28px;border:3px solid #4A525C;border-top-color:#50C6A0;border-radius:50%;animation:tj-spin 0.8s linear infinite;} @keyframes tj-spin{to{transform:rotate(360deg);}}`}</style>
        <div className="tj-spinner" />
  ...
  if (!user || window.location.pathname === '/landing') {
    return <PublicSite onAuthed={(u) => { window.history.replaceState({}, '', '/'); setUser(u); }} />;
  }
