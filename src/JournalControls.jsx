...
import {timeParts, storedTime, displayTime} from './timePickerModel';
...
export function ThemeSelect({ value, onChange, children, label, disabled }) {
  ...
  return <Popup label={label} disabled={disabled} text={options.find(item => String(item.value) === String(value))?.label}>{close => <><strong>{label}</strong><div className={entry ? 'tj-choice-grid' : ''}><ChoiceList options={options} value={value} label={label} onSelect={next => {onChange({target:{value:String(next)}}); close();}}/></div></>}</Popup>;
}
