...
export default function JournalSignalHeader({loginQuote, ...data}) {
  ...
  return <section className="tj-dashboard-welcome" aria-label={`${names[data.page]} journal insight`}>
    <div className="tj-dashboard-welcome-copy"><span>AAICOREFX / {names[data.page]?.toUpperCase()}</span><h1>{message.title}<br/><span className="tj-signal-secondary">{message.subtitle}</span></h1><p>{message.detail}</p></div>
