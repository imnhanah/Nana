...
export function buildDemoHistory() {
  ...
  const addPeriod = (type, key, selected) => {
    ...
    const summary = `Synthetic demonstration: ${selected.length} trades, ${wins} wins, ${losses} losses, ${selected.length - wins - losses} breakevens, ${(wins / selected.length * 100).toFixed(1)}% win rate, $${pnl.toFixed(2)} net P&L. Risk was 1% of running equity with a 3.2R target. Three entry models and varied confluences were used.`;
    periods.push({type, key, completed: true, content: {
      ...
      technicalWins: "Winning examples reached the planned 3.2R target.", technicalLosses: "Losing examples closed at the planned 1R stop.", technicalRules: "Keep the 1% risk cap and 3.2R target consistent.",
  ...
  for (let q = 1; q <= 4; q++) addPeriod("quarterly", `2025-Q${q}`, trades.filter(t => Math.ceil(Number(t.date.slice(5,7)) / 3) === q));
  addPeriod("annual", "2025", trades);
