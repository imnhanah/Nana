...
export default function ChallengeModeSettings({challenge, enabled}) {
  ...
  return <div className="tj-challenge-mode-settings">
    ...
    <p>{!enabled ? 'Enable Challenge and save the account first.' : loading ? 'Loading saved mode…' : !modesReady ? 'Apply challenge_modes_migration.sql to enable modes. Manual challenge controls remain available.' : !mode ? 'Manual: trades never change your level. Select a mode to automate; click it again to return to manual.' : mode==='risk' ? 'Gross P&L accumulates at each level. Reach the profit target to advance, or the risk limit to step back.' : 'Reach the gross profit target to advance. Any two losses step back; wins reset the streak and breakevens leave it unchanged.'}</p>
