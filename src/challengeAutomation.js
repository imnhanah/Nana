...
// A checkpoint freezes prior progress. Only trades closed after it can replay.
export function resetAutomation(state, trades, mode, level = state.activeLevel, now = new Date()) {
  ...
  return {activeLevel:level, statuses, notes:{...state.notes}, automation:{
    mode:validMode(mode), startedAt:localCheckpoint(now),
    excludedIds:trades.filter(t=>t.closeDate).map(t=>t.id)
  }};
}
export function replayChallenge(baseline, account) {
  ...
  const rows=(account.trades || []).filter(t=>{
    if (!t.closeDate || excluded.has(t.id) || (t.accountId && t.accountId !== account.id)) return false;
    const closed=`${t.closeDate}T${t.closeTime || '23:59'}`;
    return closed >= config.startedAt && Number.isFinite(Number(t.grossPnl));
  }).sort((a,b)=>`${a.closeDate}T${a.closeTime || '23:59'}`.localeCompare(`${b.closeDate}T${b.closeTime || '23:59'}`) || String(a.id).localeCompare(String(b.id)));
  let cents=0;
